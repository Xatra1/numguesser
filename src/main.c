...
char doc[] = "Num-Guesser is a random number guessing game written in C.\vOnly "
             "one argument may be passed to the program at a time. If multiple "
...
/* A structure of possible arguments that can be passed to the program to allow
 * for different outcomes. */
const struct argp_option options[] = {
    {"dont-show-num", 'n', 0, 0,
     "Prevents the program from displaying what the correct number was after a "
     "game over.\n"},
    {"show-exit-code", 's', 0, 0,
     "Prints the exit code when the program terminates.\n"},
    {0, 'r', 0, OPTION_ALIAS},
    {"difficulty", 'd', "1-3", 0,
     "Chooses the difficulty of the game, skipping the difficulty select "
     "phase. Accepts any value from 1-3 (inclusive).\n"},
    {"help", 'h', 0, 0,
...
/* A structure of variables that can be modfied by arguments passed to the
 * program. */
struct arguments {
...
/* Parse command line arguments and execute statements based on the argument
 * passed. */
error_t parse_opt(int key, char *arg, struct argp_state *state) {
  /* A pointer to the arguments structure that allows variables contained within
   * to be manipulated by parse_opt. */
...
/* A structure that contains the option declarations, parse function, command
 * usage documentation, and additional help strings. */
...
/* Create the difficulty selection prompt, scan for answer, and repeat until
 * answer is anything besides 'n'. */
int diffChoose() {
  ...
  fprintf(stdout,
          "You selected %s. You will have %d attempt(s) to get the number "
          "correct.\n",
          diffStr, attempts);
